...
namespace SteamKit2.CDN
{
    ...
    public sealed class Client : IDisposable
    {
        ...
        /// <exception cref="HttpRequestException">An network error occurred when performing the request.</exception>
        /// <exception cref="SteamKitWebRequestException">A network error occurred when performing the request.</exception>
        public async Task<DepotChunk> DownloadDepotChunkAsync(
            uint depotId,
            DepotManifest.ChunkData chunk,
            Server server,
            byte[]? depotKey = null,
            Server? proxyServer = null,
            ChunkDownloadProgress ctx = null)
        {
            ArgumentNullException.ThrowIfNull(server);
            ...
            var chunkData = await DoRawCommandAsync(server, string.Format("depot/{0}/chunk/{1}", depotId, chunkID), proxyServer, ctx).ConfigureAwait(false);
        ...
        public async Task<byte[]> DoRawCommandAsync(Server server, string command, Server? proxyServer, ChunkDownloadProgress? ctx = null)
        {
            ...
            try
            {
                ...
                if (ctx != null)
                {
                    using var responseStream = await response.Content.ReadAsStreamAsync(cts.Token).ConfigureAwait(false);
                    ...
                    while ((bytesRead = await responseStream.ReadAsync(buffer, 0, buffer.Length, cts.Token).ConfigureAwait(false)) > 0)
                    {
                        memoryStream.Write(buffer, 0, bytesRead);
                        totalRead += bytesRead;

                        // Update progress if there is a progress reporter and a total length
                        ctx.Progress(totalRead, totalLength, totalLength > 0 ? (double)totalRead / totalLength : -1);
                    }
